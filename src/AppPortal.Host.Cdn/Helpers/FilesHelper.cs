...
namespace AppPortal.Host.Cdn.Helpers
{
    public class FilesHelper
    {
        ...
        {
            string virtualPath = Path.Combine(pathToDelete);
            string path = hostingEnvironment.WebRootFileProvider.GetFileInfo(virtualPath).PhysicalPath;
        ...
        private void UploadWholeFile(HttpContext requestContext, List<ViewDataUploadFilesResult> statuses)
        {
            ...
            var request = requestContext.Request.Form;
                ...
                if (imageArray.Length != 0)
                {
                    ...
                    else
                    {
                        ...
                        using (MemoryStream stream = new MemoryStream(System.IO.File.ReadAllBytes(fullPath)))
                        {
                            ...
                            var ImageBit = ImageHandler.LoadImage(stream);
                            handler.Save(ImageBit, 80, 80, 10, ThumbfullPath2);

                        }

                    }
                }
                statuses.Add(UploadResult(file.FileName, Convert.ToInt32(file.Length), file.FileName));
            }
        }
        ...
        {
            var request = requestContext.Request.Form;
            if (request.Files.Count != 1) throw new FileNotFoundException("Attempt to upload chunked file containing more than one fragment per request");
        ...
        {
            String getType = MimeMapping.MimeUtility.GetMimeMapping(FileFullPath);
        ...
        public List<String> FilesList()
        {
            ...
            string virtualPath = Path.Combine(serverMapPath);
            string path = hostingEnvironment.WebRootFileProvider.GetFileInfo(virtualPath).PhysicalPath;
